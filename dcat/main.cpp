...
static void clientOnSend(int err)
{
	LOG4CPLUS_TRACE(logger(), "clientOnSend(" << err << ")");
	...
	conn->write(&buff[0], &buff[0] + sizeof(buff), &clientOnSend);
}
...
void serverOnRecv(int err, int numBytes)
{
	LOG4CPLUS_TRACE(logger(), "serverOnRecv(" << err << ", " << numBytes << ")");
	...
	conn->read(&buff2[0], &buff2[0] + sizeof(buff2), &serverOnRecv, false);
}
